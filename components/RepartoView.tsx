...
import {
  OSPITI, PERIODI, REGOLE, RIFERIMENTO, MAX_CHIAMATE_JEV,
  diariNelPeriodo, classifica, valutaLocale, pianoChiamate, urgencyScore, fmtData,
  type Periodo, type Ospite, type Diario, type DiarioValutato, type RigaClassifica,
} from '../lib/reparto';
...
const preview = (t: string, n = 80) => (t.length > n ? t.slice(0, n - 1).trimEnd() + '…' : t);
...
export default function RepartoView({ mode, onOpen }: { mode: Mode; onOpen: (o: Ospite, d: Diario) => void }) {
  ...
  const [sel, setSel] = useState<Set<string>>(() => new Set(OSPITI.map((o) => o.id)));
  ...
  const locale = useMemo(() => new Map(OSPITI.flatMap((o) => o.diari).map((d) => [d.id, valutaLocale(d)])), []);
        ...
        const worker = async () => {
          while (queue.length && !missingKey) {
            ...
            try {
              ...
              else out.set(d.id, { diario: d, punteggio: urgencyScore(d.testo, json as AnalysisResult), mode: 'jev' });
            } catch { errori++; }
      ...
      <section className="lg:col-span-5">
        <div className="flex flex-col rounded-2xl border border-slate-200 bg-white shadow-sm lg:sticky lg:top-6 lg:h-[calc(100vh-8.5rem)] lg:min-h-[520px]">
          <div className="shrink-0 border-b border-slate-100 px-4 py-2.5">
            ...
            <p className="text-xs text-slate-500">{OSPITI.length} ospiti, {OSPITI.reduce((n, o) => n + o.diari.length, 0)} diari. Riferimento temporale: ultimo diario del {fmtRif()}.</p>
          </div>
          ...
          <ul className="mt-2 max-h-[45vh] min-h-0 flex-1 space-y-1.5 overflow-y-auto px-4 pb-2 lg:max-h-none" aria-label="Ospiti">
            {OSPITI.map((o) => {
              ...
              const n = diariNelPeriodo(o, periodo).length;
              const disabled = n === 0;
              return (
                <li key={o.id}>
                  <label className={`flex cursor-pointer gap-2.5 rounded-xl border px-2.5 py-2 transition ${disabled ? 'cursor-default border-dashed border-slate-200 opacity-60' : sel.has(o.id) ? 'border-blue-200 bg-blue-50/40' : 'border-slate-200 hover:bg-slate-50'}`}>
                    <input type="checkbox" className="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-300 accent-blue-700" checked={!disabled && sel.has(o.id)} disabled={disabled || busy} onChange={() => toggle(o.id)} />
                    <div className="min-w-0 flex-1">
                      ...
                      {ultimo ? (
                        <>
                          <p className="mt-0.5 text-[11px] text-slate-500 tabular">{fmtData(ultimo)} · {ultimo.autore} · {n} {n === 1 ? 'diario' : 'diari'}</p>
                          <p className="text-xs leading-snug text-slate-600">{preview(ultimo.testo)}</p>
                        </>
                      ) : (
                        <p className="mt-0.5 text-xs text-slate-400">Nessun diario nel periodo</p>
                      )}
                    </div>
                  </label>
                </li>
          ...
          <>
            {demo && (
              <div className="animate-rise rounded-xl border border-blue-200 bg-blue-50/70 px-3 py-2.5 text-xs leading-relaxed text-blue-950">
                ...
                {nome(demo.b.r.ospite)} (PA 135/90, FC 88, pallida, sudata, confusa) è al <span className="font-semibold">{demo.b.i}° posto</span> con {demo.b.r.migliore.punteggio.score} punti.{' '}
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {righe.slice(0, 3).map((r, i) => <TopCard key={r.ospite.id} r={r} rank={i + 1} mock={isMockRun} onOpen={onOpen} />)}
            </div>

            {righe.length > 3 && (
              <ol className="divide-y divide-slate-100 overflow-hidden rounded-xl border border-slate-200 bg-white" start={4}>
                {righe.slice(3).map((r, i) => <Riga key={r.ospite.id} r={r} rank={i + 4} max={righe[0].migliore.punteggio.score} onOpen={onOpen} />)}
              </ol>
            )}
...
function TopCard({ r, rank, mock, onOpen }: { r: RigaClassifica; rank: number; mock: boolean; onOpen: (o: Ospite, d: Diario) => void }) {
  ...
  return (
    <article className={`animate-rise flex flex-col rounded-2xl border p-4 shadow-sm ${tone}`} style={{ animationDelay: `${rank * 60}ms` }}>
      ...
      {r.ultimo.id !== r.migliore.diario.id && <p className={`mt-1.5 text-[11px] ${sub}`}>Ultimo diario {fmtData(r.ultimo)}: {preview(r.ultimo.testo, 70)}</p>}
      <button onClick={() => onOpen(r.ospite, r.migliore.diario)}
        className={`mt-auto pt-3 text-left text-xs font-semibold underline-offset-2 hover:underline ${p.stato === 'rosso' ? 'text-white' : 'text-blue-800'}`}>
        Apri nel diario ospite ›
      </button>
...
function Riga({ r, rank, max, onOpen }: { r: RigaClassifica; rank: number; max: number; onOpen: (o: Ospite, d: Diario) => void }) {
  const p = r.migliore.punteggio;
  return (
    <li className="flex gap-3 px-3 py-2.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-slate-500"><span className="tabular">{fmtData(r.migliore.diario)}</span> · {preview(r.migliore.diario.testo, 110)}
          <button onClick={() => onOpen(r.ospite, r.migliore.diario)} className="ml-1.5 font-medium text-blue-700 hover:underline">Apri</button>
        </p>
      </div>
    </li>
  );
